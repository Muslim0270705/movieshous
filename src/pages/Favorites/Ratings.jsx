...
const Ratings = () => {
    ...
    const {likes} = useSelector((store) => store.cinema)
                                ...
                                <div className="favorite__logo">
                                    ...
                                    <img onClick={() => {
                                        if(likes.data.filter(el => el.id === item.id)?.length){
                                         ...
                                         className="favorite__icon" src={likes.data.filter(el => el.id === item.id)?.length ? "https://web-static.more.tv/static/icons/added-to-favorites.9491c207.svg" : "https://web-static.more.tv/static/icons/add-to-favorites.a73bfe93.svg"} alt=""/>
                                </div>
                                <Link to={`/film/${item.id}`} className="favorite__card"
                                      style={{color: "white"}}>
